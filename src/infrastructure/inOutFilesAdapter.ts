...
export class InOutFilesAdapter {
	 ...
	 */
	getArchivedPath(type: FileType, id: TestCaseId): string {
		return path.join(
			this.workspaceRoot,
			'.pahcer-ui',
			'results',
	 ...
	 */
	async loadArchived(type: FileType, id: TestCaseId): Promise<string> {
	 ...
	 * 注：解析処理はユースケース層で実装
	 */
	async archiveFiles(executionId: string): Promise<void> {
