...
- **Best ランキングフィルタ（統計情報パネル）**:
  - `Best対象`: コメント部分一致で #Best 判定に含める提出を限定（空欄=全提出）
  - `Best除外`: コメント部分一致で #Best 判定から除外
  - #Best / #Unique は選択中の比較対象ではなく、上記フィルタ後の全提出を基準に計算
...
- **保存項目**:
  ...
  - `xAxis`: X軸設定
  - `yAxis`: Y軸設定
  - `bestRankingInclude`: Best 判定の対象コメント
  - `bestRankingExclude`: Best 判定の除外コメント
